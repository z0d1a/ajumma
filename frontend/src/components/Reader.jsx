...
export default function Reader({ setHistory }) {
  ...
  // record history whenever slug/chapter changes
  useEffect(() => {
    if (!slug || !chapter) return
    setHistory(prev => {
      const filtered = prev.filter(e => e.slug !== slug)
      const next = [{ slug, chap: chapter }, ...filtered]
      return next.slice(0, 10)  // max 10 entries
    })
  }, [slug, chapter, setHistory])
  ...
  return (
    ...
    >
      ...
      <div className="flex-grow overflow-x-hidden">
        <div
          id="reader"
          className="flex flex-col items-center bg-transparent overflow-auto space-y-8 px-2 py-4 md:px-4 md:py-8"
        >
          {pages.map((p, i) => (
            <img
              key={i}
      ...
      <footer className="bg-gray-100 dark:bg-gray-900 border-t border-gray-700 px-4 py-3">
