...
export default function Reader({ setHistory }) {
  ...
  // fetch pages for this chapter and record history
  useEffect(() => {
    ...
    api
      .get(`/page?m=${encodeURIComponent(slug)}&c=${chapter}`)
      .then(res => {
        const arr = Array.isArray(res.data) ? res.data : res
        setPages(arr)
        // record into history: put this entry at front, dedupe
        setHistory(prev => {
          const filtered = prev.filter(
            item => !(item.slug === slug && item.chap === chapter)
          )
          return [{ slug, chap: chapter }, ...filtered]
        })
      })
  ...
  // fetch series detail (for next/prev)
  useEffect(() => {
    if (!slug) return
    api
      .get(`?m=${encodeURIComponent(slug)}`)
      .then(res => {
        ...
        <div className="container mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-col items-center sm:items-start text-center sm:text-left">
            ...
            <select
              value={chapter}
              onChange={e =>
                navigate(`/chapters/${slug}/${e.target.value}`)
              }
